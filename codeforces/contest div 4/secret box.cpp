...
vector<ll>arr(3),factor(3,1);
ll k,s;
cin>>arr[0]>>arr[1]>>arr[2]>>k;
s=k;
int i=0;

//equally distribute all factors
while(s>=2){
    for(ll x=2;x<=s;x++){
        if(s%x==0){
            factor[i]*=x;
...
//refactor
while(arr[0]<factor[0]){
    for(ll x=2;x<=factor[0];x++){
        if(factor[0]%x==0){
            factor[0]/=x;
            factor[1]*=x;
            break;
...
while(arr[1]<factor[1]){
    for(ll x=2;x<=factor[1];x++){
        if(factor[1]%x==0){
            factor[1]/=x;
            factor[2]*=x;
            break;
...
ll ways=0;
if(arr[0]>=factor[0] && arr[1]>=factor[1] && arr[2]>=factor[2])
ways=(arr[0]-factor[0]+1)*(arr[1]-factor[1]+1)*(arr[2]-factor[2]+1);
