...
ll h,n;
cin>>h>>n;
vector<ll> a(n),c(n),dp(h+1,LONG_LONG_MAX);
rep(n){
    cin>>a[i01];
}
rep(n){
...
dp[h]=0;
for(ll i=h;i>0 && dp[i]!=LONG_LONG_MAX;i--){
    for(ll j=0;j<n;j++){
        ll attack=a[j],cd=c[j];
        ll health=i-attack,turn=dp[i]+c[j];
        //cout<<health<<" "<<attack<<" "<<cd<<endl;
        if(health<0)health=0;
        dp[health]=min(dp[health],turn);
    }
}
nline
fout(dp)
