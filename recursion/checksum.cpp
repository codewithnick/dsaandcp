...
int checksum(vi arr,vi& fixed, int i ,int n, int sum){
    if(i==n)return 0;
    int elem =fixed[i++];
    int l=checksum(arr,fixed,i,n,sum);
    arr.push_back(elem);
    int check=accumulate(arr.begin(),arr.end(),0);
    if(check==sum)
    {
        return 1+l;
    }
    int r= checksum(arr,fixed,i,n,sum);
    return l+r;
}
