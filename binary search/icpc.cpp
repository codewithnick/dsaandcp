...
const ll N = 2e5 + 10  ;
int swaps=0;
vector <pair<int,int>> swapindex;
bool comp(pair<int,int>a,pair<int,int>b){
   if(b.first>a.first)
   {
      swaps++;
      swapindex.push_back({a.second,b.second});
      return true;
   }
   return false;
}
...
void solve() {
        ...
        cout<<swaps*3<<endl;
         for(i=0;i<swaps;i++){
            cout<<swapindex[i].first<<" "<<swapindex[i].second<<endl;
            cout<<swapindex[i].second<<" "<<swapindex[i].first<<endl;
            cout<<swapindex[i].first<<" "<<swapindex[i].second<<endl;
         }
         /*
         for(i=0;i<n;i++)
